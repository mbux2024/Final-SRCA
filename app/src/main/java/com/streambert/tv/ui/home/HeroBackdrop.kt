...
/**
 * FIXED LAYER — Full-screen crossfading backdrop with Ken Burns effect.
 *
 * Fills the ENTIRE screen. Never scrolls. Crossfades (~700ms) whenever
 * [currentFeaturedItem] changes. Subtle Ken Burns scale (1.0→1.05, 12s).
 */
@Composable
fun HeroBackdrop(
...
/**
 * Focus/rotation state manager.
 *
 * - focusedItem non-null → debounce 200ms then show that item's backdrop.
 * - focusedItem null (idle) → auto-rotate through pool every intervalMs.
 * - 2s cooldown after focus clears before resuming rotation.
 */
@Composable
fun rememberFeaturedItem(
    ...
    // Debounce focus changes (200ms) to prevent flicker on fast D-pad scrubbing
